...
def ArgParse():
    ...
    parser.add_argument('--train_dir', help='Train-source dataset folder', type=str, default='train images path')
    parser.add_argument('--test_dir', help='Test-source dataset folder', type=str, default='valid images path')
    # Misc
...
def main(args):
    # Create datasets
    train_dataset, test_dataset = create_dataset(args.train_dir,
                                                 args.test_dir,
                                                 args.batch_size)
    ...
    # Compile model
    cut.compile(G_optimizer=SAM(tf.keras.optimizers.Adam(learning_rate=lr_schedule, beta_1=args.beta_1, beta_2=args.beta_2)),
                F_optimizer=SAM(tf.keras.optimizers.Adam(learning_rate=lr_schedule, beta_1=args.beta_1, beta_2=args.beta_2)),
                D_optimizer=SAM(tf.keras.optimizers.Adam(learning_rate=lr_schedule, beta_1=args.beta_1, beta_2=args.beta_2)), )
    ...
    checkpoint_callback = tf.keras.callbacks.ModelCheckpoint(filepath=checkpoint_dir + '/{epoch:03d}',
                                                             period=args.save_n_epoch, save_weights_only=True,
                                                             verbose=1)
...
def interclassshuffle(dataset_path):
    src_dataset = glob.glob(os.path.join(dataset_path, '*.png'))
    tar_dataset = glob.glob(os.path.join(dataset_path, '*.png'))
